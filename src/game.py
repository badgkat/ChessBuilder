...
class Game:
    def __init__(self, screen):
        ...
        self.last_move = None  # (from_sq, to_sq) tuple for highlighting
    ...
    def create_time_control_options(self):
        ...
        text_surfaces = [
            self.font.render(label, True, (0, 0, 0)) for label, *_ in choices
        ]
        ...
        overlay_h = (
            (btn_h * len(choices)) + (btn_spacing * (len(choices) - 1)) + margin * 2
        )
    ...
    def load_images(self):
        ...
        for color in ["white", "black"]:
            for p in pieces:
                try:
                    ref = importlib.resources.files(assets).joinpath(f"{color}_{p}.png")
                    with importlib.resources.as_file(ref) as path:
                        img = pygame.image.load(str(path))
                        img = pygame.transform.scale(
                            img, (board.SQUARE_SIZE, board.SQUARE_SIZE)
                        )
    ...
    def new_game(self):
        self.board = [
            [None for _ in range(board.BOARD_SIZE)] for _ in range(board.BOARD_SIZE)
        ]
    ...
    def restore_purchase_state(self):
        if self.pre_purchase_state:
            ...
            self.valid_purchase_placement = self.pre_purchase_state[
                "valid_purchase_placement"
            ][:]
            self.error_message = self.pre_purchase_state["error_message"]

    def restore_promotion_state(self):
            ...
            for c in range(board.BOARD_SIZE):
                ...
                if piece and piece.color == color:
                    moves = board.get_valid_moves(
                        piece, (r, c), self.board, self.en_passant
                    )
                    for m in moves:
    ...
    def move_piece(self, src, dst):
        ...
        if (
            ...
            captured_pawn = self.board[captured_pos[0]][captured_pos[1]]
            if captured_pawn:
                mover.gold += captured_pawn.gold + board.PIECE_COST.get(
                    captured_pawn.type, 0
                )
    ...
    def capture_piece(self, src, dst):
        ...
        if target:
            mover.gold += target.gold + board.PIECE_COST.get(target.type, 0)
        self.board[dr][dc] = mover
        self.board[sr][sc] = None
        ...
        # Promotion check for capturing pawns
        if mover.type == "P":
            final_rank = (mover.color == "white" and dr == 0) or (
                mover.color == "black" and dr == board.BOARD_SIZE - 1
            )
    ...
    def create_purchase_options(self, row, col):
        ...
        overlay_w = (
            (btn_size * len(purchase_list))
            + (btn_spacing * (len(purchase_list) - 1))
            + margin * 2
        )
    ...
    def create_promotion_options(self):
        ...
        overlay_w = (
            (btn_size * len(promo_list))
            + (btn_spacing * (len(promo_list) - 1))
            + margin * 2
        )
        overlay_h = btn_size + margin * 2
    ...
    def create_pause_menu_options(self):
        ...
        # Calculate minimum width needed for text
        text_surfaces = [
            self.font.render(label, True, (0, 0, 0)) for label, _ in options
        ]
        min_btn_width = max(surf.get_width() for surf in text_surfaces) + margin * 4
        btn_w = max(200, min_btn_width)  # minimum width of 200px
        ...
        overlay_w = btn_w + margin * 2
        overlay_h = (
            (btn_h * len(options)) + (btn_spacing * (len(options) - 1)) + margin * 2
        )
    ...
    def draw_board(self):
        ...
        for r in range(board.BOARD_SIZE):
            for c in range(board.BOARD_SIZE):
                ...
                if self.last_move and (r, c) in self.last_move:
                    lm_overlay = pygame.Surface(
                        (board.SQUARE_SIZE, board.SQUARE_SIZE), pygame.SRCALPHA
                    )
                    lm_overlay.fill(board.LAST_MOVE_COLOR)
                    self.screen.blit(lm_overlay, (rect_x, rect_y))

                # highlight king in check
                piece_at = self.board[r][c]
                if (
                    piece_at
                    and piece_at.type == "K"
                    and self.is_in_check(piece_at.color)
                ):
                    chk_overlay = pygame.Surface(
                        (board.SQUARE_SIZE, board.SQUARE_SIZE), pygame.SRCALPHA
                    )
                    chk_overlay.fill(board.CHECK_HIGHLIGHT_COLOR)
                    self.screen.blit(chk_overlay, (rect_x, rect_y))

                # highlight selection
                if self.selected_piece_pos == (r, c):
                    select_overlay = pygame.Surface(
                        (board.SQUARE_SIZE, board.SQUARE_SIZE), pygame.SRCALPHA
                    )
                ...
                if (r, c) in self.valid_purchase_placement:
                    pygame.draw.rect(self.screen, board.PURPLE, rect, 3)
                ...
                if piece:
                    ...
                    if piece.gold > 0:
                        ...
                        pygame.draw.circle(
                            self.screen,
                            board.GOLD_CIRCLE_COLOR,
                            (center_x, center_y),
                            radius,
                        )
                        gold_txt = self.font.render(
                            str(piece.gold), True, board.GOLD_TEXT_COLOR
                        )
        ...
        rank_rect = pygame.Rect(
            0,
            board.BOARD_HEIGHT,
            board.BOARD_WIDTH + board.MARGIN_LEFT,
            board.MARGIN_BOTTOM,
        )
        ...
        panel_rect = pygame.Rect(
            board.BOARD_WIDTH + board.MARGIN_LEFT,
            0,
            board.RIGHT_PANEL_WIDTH,
            board.BOARD_HEIGHT + board.MARGIN_BOTTOM,
        )
        ...
        for line in lines:
            ...
            self.screen.blit(
                txt, (board.BOARD_WIDTH + board.MARGIN_LEFT + 10, y_offset)
            )
            y_offset += line_height

        bottom_rect = pygame.Rect(
            0,
            board_y + board.BOARD_HEIGHT + board.MARGIN_BOTTOM,
            board.WINDOW_WIDTH,
            board.BOTTOM_PANEL_HEIGHT,
        )
        ...
        if self.error_message:
            ...
            self.screen.blit(
                err_txt, (10, board.BOARD_HEIGHT + board.MARGIN_BOTTOM + 34)
            )
        ...
        if self.purchase_mode and self.purchase_overlay_rect:
            ...
            for rect_, p_type in self.purchase_options:
                ...
                if self.purchase_king_color:
                    ...
                    if icon:
                        small_icon = pygame.transform.scale(
                            icon, (rect_.width, rect_.height)
                        )
        ...
        if self.game_over:
            ...
            self.screen.blit(
                ts,
                ts.get_rect(center=(board.WINDOW_WIDTH // 2, board.WINDOW_HEIGHT // 2)),
            )
        ...
        self.screen.blit(
            turn_txt,
            (board.MARGIN_LEFT + 10, board.BOARD_HEIGHT + board.MARGIN_BOTTOM + 10),
        )
    ...
    def handle_board_click(self, pos):
        if self.time_control_mode:
            if (
                self.time_control_overlay_rect
                and self.time_control_overlay_rect.collidepoint(pos)
            ):
        ...
        if self.promotion_mode:
            if (
                self.promotion_overlay_rect
                and self.promotion_overlay_rect.collidepoint(pos)
            ):
                for rect_, p_type in self.promotion_options:
        ...
        if self.purchase_mode and self.purchase_overlay_rect:
            if self.purchase_overlay_rect.collidepoint(pos):
                            ...
                            for dr2 in [-1, 0, 1]:
                                for dc2 in [-1, 0, 1]:
                                    ...
                                    if (
                                        ...
                                        if p_type == "P" and (
                                            nr == 0 or nr == board.BOARD_SIZE - 1
                                        ):
                                            continue
                                        self.board[nr][nc] = board.Piece(
                                            p_type, king.color
                                        )
                                        if not self.is_in_check(king.color):
                                            self.valid_purchase_placement.append(
                                                (nr, nc)
                                            )
                ...
                self.restore_purchase_state()
                self.purchase_mode = False
            return

        # check if clicked on the board
        if not (
            board_x <= pos[0] < board_x + board.BOARD_WIDTH
            and board_y <= pos[1] < board_y + board.BOARD_HEIGHT
        ):
            return
        ...
        if self.placement_mode:
            if (row, col) in self.valid_purchase_placement:
                ...
                self.board[row][col] = board.Piece(
                    self.purchase_selected_type, king.color
                )
            ...
            if (row, col) == sp:
                ...
                if piece.type == "P":
                    ...
                    self.move_log.append(
                        f"{piece.type}+{board.square_to_notation(row, col)}"
                    )
                    self.halfmove_clock = 0
                    self.selected_piece_pos = None
                    self.clear_valid_actions()
                ...
                if (row, col) in self.valid_move_squares:
                    ...
                    if src_piece.gold > 0:
                        ...
                        src_piece.gold = 0
                        self.move_log.append(
                            f"{src_piece.type}G{board.square_to_notation(row, col)}"
                        )
                        self.halfmove_clock += 1
                        self.selected_piece_pos = None
                        self.clear_valid_actions()
